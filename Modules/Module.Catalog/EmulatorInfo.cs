...
/// <summary>
/// An emulator whose per-game compatibility list is ingested, mapped to a primary <see cref="Console"/>
/// and the join key (<see cref="CompatMatchKind"/>) its data exposes. <see cref="ExtraConsoles"/> lets
/// an emulator span more than one console (Dolphin → GameCube + Wii); it defaults to just
/// <see cref="Console"/>. <see cref="Consoles"/> is the full set — used to console-gate the name join,
/// where keys (titles) collide across consoles, unlike globally-unique serials/Title IDs.
/// </summary>
public sealed record EmulatorInfo(string Id, string Name, string Console, CompatMatchKind MatchKind,
    IReadOnlyList<string>? ExtraConsoles = null)
{
    public IReadOnlyList<string> Consoles => ExtraConsoles ?? [Console];
}
