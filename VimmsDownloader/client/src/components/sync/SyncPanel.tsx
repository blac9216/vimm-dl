      ...
      <div className="flex items-center gap-2 px-3 sm:px-6 py-2.5 bg-surface/30 border-b border-border/20 flex-wrap">
        ...
        <button onClick={() => cancelMutation.mutate()}
          className="px-3.5 py-1 text-xs font-medium rounded bg-surface-2/40 text-text-3
            border border-border/30 hover:bg-error/10 hover:text-error hover:border-error/20">
          Cancel
        </button>
      </div>

      {syncData?.error && (
        <div className="mx-3 sm:mx-6 mt-2 p-2 bg-error/8 border border-error/20 rounded text-xs text-error">
          {syncData.error}
        </div>
      )}
