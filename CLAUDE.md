...
- **Jobs tab** (`components/jobs/`) — everything that isn't a download (#259). `JobsPanel` merges two feeds client-side through one `JobRow`: live conversions from the SignalR `ConvertStatus` state (`convStatuses`/`convStartTimes`, PS3 + Wii U, multi-format rows grouped per game, stop = `POST /api/ps3/action` `{action:'abort'}`) and the running background jobs of `GET /api/jobs` (`useJobs` polls 2s while any run, 15s idle; stop = `POST /api/jobs/{kind}/cancel`). Row = kind-colored left border + 104px kind badge + name/message + 140px progress bar (indeterminate when the job reports no total) + status badge + elapsed + stop; elapsed is rendered **only while running** (the API's `elapsedMs` keeps growing after a run ends). Kind colors live in `lib/jobs.ts`. Header carries the job count, the "Clear finished" dismissal for terminal conversion rows, and the **Run job** trigger group — the eight catalog jobs (Sync / Scan / Verify / Compat / Vimm + console scope / IGDB descriptions / IGDB rank / RetroAchievements) moved here out of the Library's `⋯` menu, each disabled while its own gate reports running. Tab count pill = running background jobs + non-terminal conversions.
