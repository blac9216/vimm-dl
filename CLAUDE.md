...
- **Library tab** (`components/library/`) — three-column master/detail browse (#257): `ConsoleRail` (212px; color code chip + console name + count, "All consoles" first), `GameList` (430px; search, availability + sort segments, advanced-filters popover for 1G1R / English / hide-demos / search-mode / emulator + compat status, batch-select + curation sub-bar, 100-per-page footer, and the catalog job triggers in a `⋯` overflow menu until L5 moves them to Jobs), `GameDetailPane` (cover, console pill, compat / Vimm / rank chips, 16:9 title-screen box, IGDB description, format chips with owned = green ✓, Download → "✓ Queued" / owned status). View state (filters + page + selected game) persists to `localStorage` (`vimm:library-filters`); `SetsDialog` (manage archive.org sets, also reachable from Settings → Archive); `FormatPickerDialog` (choose the Vimm download format when a bound game offers more than one). Columns stack below `sm` (rail → chip strip, detail → drill-in).
