...
- **Module.Catalog** — The canonical catalog + Vimm binding (web-free; persists via `ICatalogStore`). `ClrMameProParser` + `CatalogSyncService` (fetch No-Intro/Redump DATs from the libretro-database mirror), `CatalogSystems` (the consoles synced — DAT name + group + EmuDeck folder), `CatalogMatcher` (name normalization / file match), `Dedup` (1G1R title-key + parent selection), the **compat seam** (`ICompatSource` adapters — `Rpcs3CompatSource` JSON + `Pcsx2CompatSource`/`DuckStationCompatSource` YAML via the AOT `YamlScanner` — feeding the `CompatSources`/`Emulators` registries with a per-emulator `CompatMatchKind`, normalized to `CompatStatuses` via `CompatKeys`), `Crc32`, and the Vimm layer: `VimmSystems` (catalog console → Vimm vault code) + `VimmVaultParser` (list rows, inline `media` JSON hashes, `hashes2.php`, `dl_format`).
