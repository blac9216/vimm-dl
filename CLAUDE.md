...
- **Catalog/source host services** — `CatalogRepository.cs` (implements `ICatalogStore`, all catalog SQL incl. the Vimm binding), `CatalogSyncService` (wired in `Program.cs` over the `libretro` client), `CatalogScanService` (owned scan of `completed/`), `CatalogVerifyService` (hash-based verify; skips compressed disc-image formats as unverifiable rather than hashing their container bytes, reporting the count — #284), `CompatSyncService` (per-emulator compat via `CompatSources`), `CatalogResolveService` (archive→Vimm download resolution, consulting the set index first — #289), `VimmSyncService` (per-console Vimm hash scrape/binding), `SetIndexService` (per-console archive.org set-content index + hash/name-match onto catalog games — #289), `DefaultSets.cs` (seeded RomGoGetter archive sets), `ArchiveAuth.cs` (Internet Archive S3 "LOW" auth via a `DelegatingHandler`). The concrete `SourceRegistry` lives in Module.Download/Sources, built from DI.
- **Endpoints/** — `FileEndpoints` (merged `/api/data` with pipeline trace), `DownloadEndpoints`, `MetadataEndpoints`, `CatalogEndpoints` (+ the `BackgroundJobGate` single-flight base & `Catalog*State` markers), `JobsEndpoints` (`GET /api/jobs` + `POST /api/jobs/{kind}/cancel` — L1 #255, reads the same `BackgroundJobGate` instances via `JobsOps`), `Ps3Endpoints`, `SyncEndpoints`, `SettingsEndpoints`, `EventEndpoints`, `MetricsEndpoints`. **47 endpoints total** (enumerated in the API Endpoints table below — that table is the source of truth for the count).
...
- `DatabaseMigrator.cs` — runs embedded SQL files from `Migrations/*.sql` in order (auto-discovered by manifest-resource name; currently through **032**)
...
- `catalog_game` (id, system_id, name, region, serial, serial_key, languages, title_key, is_parent, **vault_id**, **vimm_match**, **archive_match**, description, **igdb_rating**, **igdb_rating_count**, **ra_players**, **rank_score**) — one entry per title/region/revision
...
- `catalog_set` (id, name, console, …) + `catalog_set_link` (id, set_id, url, position, **indexed_at**) — per-console archive.org download sets
- `catalog_set_file` (id, link_id, name, size, crc, md5, sha1, game_id, match_kind) — a set link's indexed archive.org file listing (#289); `game_id`/`match_kind` record which catalog game (if any) the file was hash/name-matched to
...
- `library_hidden_consoles` = "" — CSV of console slugs hidden from the Library's console rail and "All consoles" browse (Settings → Library); a browse preference only, `GET /api/catalog/games` honors it only when no explicit `?console=` is given
...
- **Owned + verify + compat** — `CatalogScanService` records which games exist under `completed/` (`catalog_owned`); `CatalogVerifyService` checks file hashes against `catalog_rom` (skipping compressed disc images as unverifiable rather than mis-hashing them, with the skip count reported — #284); `CompatSyncService` imports each registered emulator's compatibility (`CompatSources`), joined to a game by serial/title_id/name per the emulator's `match_kind`.
- **Download sets** — a "set" is a named, per-console list of archive.org links (`catalog_set` + `catalog_set_link`). Defaults are seeded once from `DefaultSets` (ported from RomGoGetter). Managed via `/api/catalog/sets` CRUD (and the Library "Sources" dialog / Settings → Sources); adding/updating/deleting a set invalidates that set's indexed `catalog_set_file` rows + any `archive_match` markers they produced (no auto re-index — the Jobs tab re-runs it).
- **Set-content index** (`POST /api/catalog/sets/index?console=`, #289) — `SetIndexService` lists every configured set link's archive.org contents once (`ICatalogSource.ListFilesAsync`, which now carries each file's `crc32`/`md5`/`sha1` when archive.org publishes them) and persists the listing to `catalog_set_file`, so the Library can show archive availability and the queue path can resolve without a live listing per request. **Matches hash-first (SHA1→MD5→CRC) against `catalog_rom`, falling back to the `CatalogMatcher` filename-stem rule** (same as on-disk ownership matching) when a file carries no hash. The winning match (by that same priority, across every link on the console) is applied to `catalog_game.archive_match` — `'sha1'`/`'md5'`/`'crc'`/`'name'` (matched), `'none'` (indexed, unmatched), or `NULL` (not yet indexed) — mirroring `vimm_match`'s shape. Throttle-free, per-console, cancellable (`CatalogSetIndexState : BackgroundJobGate`, kind `set-index`).
...
- **Download resolution** (`CatalogResolveService.ResolveForQueueAsync`, used by `POST /api/catalog/games/{id}/queue?format=`) — consults the set index first (`catalog_game.archive_match` + the matched `catalog_set_file` row builds a download URL directly, no listing call); if the game isn't indexed/matched, falls back to a live listing of archive.org sets (match a game's file across the console's set links); if neither, falls back to the bound Vimm vault URL (`https://vimm.net/vault/{vault_id}`) with the requested format if Vimm offers it, else the first available. Queues `(url, format, source)`; archive uses format 0. Returns 404 when neither source has it.
...
- Tabs: Library (always visible, default tab), Downloads, Jobs, Metrics (Jobs + Metrics always visible), Sync (beta flag), Events (developer flag), Import (beta flag), Settings
...
- **Jobs tab** (`components/jobs/`) — everything that isn't a download (#259). An Active/Completed segmented sub-view (#292, styled like `DownloadsPanel`'s #258 switcher) sits under the header; both sub-views render through the same `JobRow`. Active = live conversions from the SignalR `ConvertStatus` state (`convStatuses`/`convStartTimes`, PS3 + Wii U, multi-format rows grouped per game, stop = `POST /api/ps3/action` `{action:'abort', platform}`, `platform` from the event's stamped pipeline origin so the route resolves the matching pipeline via `DownloadQueue.GetPipeline` instead of always hitting PS3's — #274) that haven't reached a terminal phase, plus the running background jobs of `GET /api/jobs` (`useJobs` polls 2s while any run, 15s idle; stop = `POST /api/jobs/{kind}/cancel`). Completed = terminal (done/error/skipped) conversion rows plus each job kind's last finished run, rendered via `CompletedJobRow` with an outcome badge (completed/failed/cancelled) and the backend's frozen `lastDurationMs`/`lastCompletedAt` (never the still-growing `elapsedMs`, which stays running-only). Row = kind-colored left border + 104px kind badge + name/message + 140px progress bar (indeterminate when the job reports no total; a completed row shows a "Finished Xm ago" label there instead) + status badge + elapsed/duration + stop. Kind colors live in `lib/jobs.ts`. Header carries the sub-view switcher, the job count (active work only), the **Run job** trigger group — the nine catalog jobs (Sync / Scan / Verify / Compat / Vimm + console scope / Index sets / IGDB descriptions / IGDB rank / RetroAchievements) moved here out of the Library's `⋯` menu, each disabled while its own gate reports running, with console options in the Vimm scope `<select>` labeled by `displayName` — and, only in the Completed sub-view, "Clear finished" (dismisses both terminal conversion rows and finished job-kind rows; client-side for both). Tab count pill unchanged = running background jobs + non-terminal conversions.
- **Library tab** (`components/library/`) — three-column master/detail browse (#257): `ConsoleRail` (212px; color code chip + console name + count, "All consoles" first, omits consoles hidden via Settings → Library (#311) — its "All consoles" count is the sum of the non-hidden ones, matching what `GET /api/catalog/games` excludes server-side when no `?console=` is given; the persisted console selection falls back to "All consoles" if it becomes hidden), `GameList` (430px; search, availability + sort segments, advanced-filters popover for 1G1R / English / hide-demos / search-mode / emulator + compat status, batch-select + curation sub-bar, 100-per-page footer, and a "Sources" button for `SetsDialog` — the catalog job triggers that shared that slot moved to the Jobs tab in #259), `GameDetailPane` (cover, console pill, compat / Vimm / Archive / rank chips — the Archive chip mirrors Vimm's, tooltip naming the match kind (hash or file name) from `archiveMatch` (#289), 16:9 title-screen box, IGDB description, format chips with owned = green ✓, Download → "✓ Queued" / owned status). All three art surfaces (list thumb, detail cover, title-screen box) render through `shared/CatalogThumb` (`?type=boxart|title`, per-image 404 fallback to the generated gradient). View state (filters + page + selected game) persists to `localStorage` (`vimm:library-filters`); `SetsDialog` (manage archive.org sets, also reachable from Settings → Sources); `FormatPickerDialog` (choose the Vimm download format when a bound game offers more than one). Columns stack below `sm` (rail → chip strip, detail → drill-in).
- **Settings tab** (`components/settings/`) — sidebar layout (#264): a 208px section nav (PS3 Conversion · Sources · Catalog · Library · Integrations · Feature Flags) beside a 640px content pane holding one card per section, rows divided by `border-inner`. `SettingsControls.tsx` holds the row primitives (`SettingsCard`/`SettingsRow`/`SettingsStackRow`/`SettingsSubHeader`, the 38×20 gradient `ToggleRow`, the console-chip `ConsoleToggleRow` (#311), the 26×26 `StepperRow` clamped at min/max, and the select/input/button recipes); `SettingsPanel.tsx` maps every settings key to a section. Section mapping (epic #254 decision 5 — nothing dropped): **PS3 Conversion** = `ps3_default_format`, `ps3_parallelism`, `ps3_preserve_archive`, the three `rename_*` toggles, plus a "Wii U" sub-block for `wiiu_common_key`; **Sources** = `archive_parallelism` / `archive_retries` / `archive_idle`, both `archive_s3_*` keys, "Manage download sets" → `SetsDialog`; **Catalog** = `catalog_dat_source`; **Library** (#311) = `library_hidden_consoles` — one `ConsoleToggleRow` per catalog console (color chip + name + game count, ON = visible), sourced from the same `GET /api/catalog/consoles` query the Library rail uses, with a "hidden N of M" summary and a "show all" reset; **Integrations** = `igdb_client_id` / `igdb_client_secret` / `ra_api_key`; **Feature Flags** = the three `feature_*` toggles. Every control persists immediately (per-key `POST /api/settings`, save-on-blur for text — no save button). The sidebar collapses to a horizontal section switcher below `sm`.
...
- Settings UI has a Feature Flags **section** in the sidebar (`components/settings/`) with one toggle per flag
- Two tiers: Beta (Import, Sync) and Developer (Events)
...
| GET | `/api/jobs` | Unified background-job surface: one row per kind with progress/message/elapsed + last-run outcome/duration (#292) |
...
| GET | `/api/catalog/games` | Paged/filtered game browse (carries `vimmMatch` + per-emulator `compat`; `?emulator=&compat=` filter); excludes `library_hidden_consoles` unless `?console=` is explicit (#311) |
...
| POST | `/api/catalog/vimm-seed` | Seed the catalog FROM Vimm where no public DAT exists (`?console=`, background) |
...
| POST | `/api/catalog/sets/index` | Index every configured set link's archive.org contents + hash/name-match onto the catalog (`?console=`, background) |
| POST | `/api/ps3/convert` | Convert all or single |
| POST | `/api/ps3/action` | Mark-done or abort; pipeline-generic (body `platform`, resolved via `DownloadQueue.GetPipeline`), PS3 default when omitted |
